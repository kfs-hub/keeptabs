  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-white/40 text-sm mt-1">The official group crime rankings.</p>
      </div>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              sortKey === btn.key
                ...
                : 'bg-white/5 text-white/50 hover:bg-white/8 hover:text-white border border-white/10'
            }`}
          >
      ...
      {sorted.length >= 3 && (
        <div className="grid grid-cols-3 gap-3 items-end">
          ...
          <motion.div
            ...
            className="glass-card rounded-2xl p-4 text-center border border-white/10 h-40 flex flex-col items-center justify-end pb-4"
          >
            ...
            <p className="text-xs font-semibold text-white mt-1 truncate w-full px-1">{sorted[1].displayName}</p>
            <p className="text-xs text-white/40">{formatCurrency(sorted[1].totalOwed, currency)}</p>
          </motion.div>
          ...
          <motion.div
            ...
            className="glass-card rounded-2xl p-4 text-center border border-violet-500/30 bg-violet-500/5 h-52 flex flex-col items-center justify-end pb-4"
          >
            ...
            <p className="text-sm font-bold text-white mt-1 truncate w-full px-1">{sorted[0].displayName}</p>
            <p className="text-xs text-violet-400 font-semibold">{formatCurrency(sorted[0].totalOwed, currency)}</p>
            <p className="text-[10px] text-white/30 mt-0.5">{getLabel(0, sorted[0], settings, sortKey)}</p>
          </motion.div>
          ...
          <motion.div
            ...
            className="glass-card rounded-2xl p-4 text-center border border-white/10 h-36 flex flex-col items-center justify-end pb-4"
          >
            ...
            <p className="text-xs font-semibold text-white mt-1 truncate w-full px-1">{sorted[2].displayName}</p>
            <p className="text-xs text-white/40">{formatCurrency(sorted[2].totalOwed, currency)}</p>
          </motion.div>
        </div>
      )}
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        {/* Column headers */}
        <div className="grid grid-cols-[40px_1fr_90px_90px_80px] gap-2 px-5 py-3 border-b border-white/5 text-xs text-white/25 font-medium uppercase tracking-wider">
          <span>#</span>
          <span>Member</span>
            ...
            {sorted.map((entry, index) => {
              ...
              return (
                <motion.div
                  ...
                  layout
                  variants={row}
                  className={`grid grid-cols-[40px_1fr_90px_90px_80px] gap-2 items-center px-5 py-3.5 border-b border-white/5 last:border-0 transition-colors ${
                    isMe ? 'bg-violet-500/5' : 'hover:bg-white/2'
                  }`}
                >
                  {/* Rank */}
                  ...
                  <Link href={`/members/${entry.userId}`} className="flex items-center gap-3 min-w-0 group">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span className={`text-sm font-medium group-hover:text-violet-300 transition-colors ${isMe ? 'text-violet-300' : 'text-white'}`}>
                          {entry.displayName}
                        </span>
                        {isMe && <Badge variant="default" className="text-[9px] py-0 px-1.5">You</Badge>}
                      </div>
                      {label && <p className="text-xs text-white/30 truncate">{label}</p>}
                      {entry.cleanStreak > 0 && (
                  ...
                  {/* Fine count */}
                  <div className="text-right">
                    <span className="text-sm text-white/40">{entry.fineCount}</span>
                  </div>
                </motion.div>
              )
            })}
