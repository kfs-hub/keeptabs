  ...
  if (fines.length === 0) {
    return (
      <div className="glass-card rounded-2xl p-8 text-center">
        ...
        <p className="text-white/50">No fines yet.</p>
        <p className="text-white/30 text-sm">Somehow you guys are behaving.</p>
      </div>
    )
  }
  ...
  return (
    <div className="glass-card rounded-2xl overflow-hidden">
      <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between">
        ...
        <Link href="/fines" className="text-xs text-violet-400 hover:text-violet-300 flex items-center gap-1 transition-colors">
      ...
      <motion.div variants={container} initial="hidden" animate="show">
        {fines.map((fine) => (
          <motion.div
            key={fine.id}
            variants={item}
            className="flex items-center gap-3 px-5 py-3.5 border-b border-white/5 last:border-0 hover:bg-white/3 transition-colors"
          >
